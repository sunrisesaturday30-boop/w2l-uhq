...
class W2l:
    def __init__(self, face, audio, checkpoint, nosmooth, resize_factor, pad_top, pad_bottom, pad_left, pad_right, face_swap_img, temp_dir=None):
        ...
        self.checkpoint_path = os.path.join(os.path.dirname(self.wav2lip_folder), 'weights', 'wav2lip', self.checkpoint + '.pth')
    ...
    def face_detect(self, images):
        ...
        for rect, image in zip(predictions, images):
            if rect is None:
                print("Hum : " + str(n))
                cv2.imwrite(os.path.join(self.temp_dir, 'faulty_frame.jpg'),
    ...
    def execute(self):
        ...
        if not self.audio.endswith('.wav'):
            ...
            command = [self.ffmpeg_binary, "-y", "-i", self.audio, "-strict", "-2",
                       os.path.join(self.temp_dir, "temp.wav")]

            self.execute_command(command)
            self.audio = os.path.join(self.temp_dir, 'temp.wav')
        ...
        for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
            ...
            if i == 0:
                ...
                out = cv2.VideoWriter(os.path.join(self.temp_dir, 'result.avi'),
        ...
        command = [self.ffmpeg_binary, "-y", "-i", self.audio, "-i", os.path.join(self.temp_dir, 'result.avi'),
